...
class Model(nn.Module):
    ...
    def get_score(self, x, y_lists):
        ...
        for i in range(len(self.p_layers) - 1):
            log_p_all += self.log_prob(
                self.p_layers[i],
                y_lists[len(self.p_layers) - i - 1],
                y_lists[len(self.p_layers) - i - 2],
            )
    ...
    def sample_q(self, x):
        ...
        for i in range(len(self.p_layers)):
            log_p_all += self.log_prob(
                self.p_layers[i],
                samples[len(self.p_layers) - i],
                samples[len(self.p_layers) - i - 1],
            )
    ...
    def jsa_loss_cache(self, x, n_samples, index, caches):
        ...
        for i in range(len(index)):
            ...
            if mod != 0:
                ...
                caches[index[i]] = [
                    samples[k][new_ind].cpu() for k in range(len(self.layers))
                ]
        ind = torch.LongTensor(ind_all).cuda()
        log_q = torch.gather(log_q, dim=0, index=ind)
        log_p = torch.gather(log_p, dim=0, index=ind)
