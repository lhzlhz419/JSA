...
trainset = MNIST(
    fname="../data/mnist_salakhutdinov.pkl.gz",
    which_set="train",
    preproc=[],
    n_datapoints=50000,
)
...
if args.method == "jsa":
    if args.net == "nonlinear":
        ...
        caches = [
            [torch.FloatTensor(np.random.randint(0, 2, 200)) for _ in range(1)]
            for _ in range(50000)
        ]
    elif args.net == "linear2":
        caches = [
            [torch.FloatTensor(np.random.randint(0, 2, 200)) for _ in range(2)]
            for _ in range(50000)
        ]
...
for ep in range(n_epoch):
    ...
    if (ep + 1) % valid_every == 0:
        ...
        print(
            "ep:%d cost:%.3f train valid test NLL:%.3f %.3f %.3f"
            % (ep + 1, time.time() - begin, train_nll, valid_nll, test_nll)
        )
