...
# Apache 2.0.
# This script contrains JSA for categorical MNIST experiment.
...
parser.add_argument(
    "--nsamples", "-n", type=int, default=20, help="particles number for training"
)
parser.add_argument("--name", default="jsa", help="model name")
...
def bernoulli_loglikelihood(b, logits):
    ...
    """
    return b * (-tf.nn.softplus(-logits)) + (1 - b) * (
...
def get_loss(sess, data, total_batch):
    ...
    for j in range(total_batch):
        ...
        cost_eval.append(sess.run(gen_loss1, {x: xs, ntimes: 1}))
...
def mis_no_cache(sess, xs):
    samples, logw = sess.run([b_sample_out, gen_loss1], {x: xs, ntimes: n + 1})
    logw = -logw.reshape(n, batch_size)
    inds = np.arange(0, batch_size * (n)).reshape(n, batch_size)
    ind_cur = inds[0]
    w_cur = logw[0]
    inds_choose = []
    for i in range(1, n):
        u = np.random.uniform(size=batch_size)
        cond = u < np.exp(logw[i] - w_cur)
        ind_cur = ind_cur * (1 - cond) + inds[i] * cond
        inds_choose += [int(i) for i in ind_cur]
    choose_sample = samples[inds_choose]
    return choose_sample


def mis_cache(sess, xs, ys, update=True):
    cache_sample = caches[ys]
    cache_logw = sess.run(gen_loss0, {x: xs, ntimes: 1, b_sample_in: cache_sample})
    samples, logw = sess.run([b_sample_out, gen_loss1], {x: xs, ntimes: n})
    logw = -np.concatenate([cache_logw, logw], 0).reshape(n + 1, batch_size)
    samples = np.concatenate([cache_sample, samples], 0)
    inds = np.arange(0, batch_size * (n + 1)).reshape(n + 1, batch_size)
    ind_cur = inds[0]
    w_cur = logw[0]
    inds_choose = []
    for i in range(1, n + 1):
        ...
        inds_choose += [int(i) for i in ind_cur]
    if update:
        caches[ys] = samples[[int(i) for i in ind_cur]]
    choose_sample = samples[inds_choose]
    return choose_sample
...
prior_logit0 = tf.get_variable(
    "p_b_logit", dtype=tf.float32, initializer=tf.zeros([n_cv, n_class])
)

x = tf.placeholder(tf.float32, [None, x_dim])
x_binary = tf.to_float(x > 0.5)
...
gen_loss0 = fun(x_binary, b_sample_in, prior_logit0, z_concate)
gen_loss1 = fun(x_binary, b_sample_out, prior_logit0, z_concate)
nll_loss = tf.reshape(gen_loss1, [n_for_nll, -1])
...
inf_grads = tf.gradients(-gen_loss, inf_vars)
...
if log_variance:
    ...
    flat_qgrad = tf.concat(
        [tf.reshape(grad, [-1]) for grad in inf_grads if grad is not None], 0
    )
    flat_pgrad = tf.concat(
        [tf.reshape(grad, [-1]) for grad in list(gen_grads) if grad is not None], 0
    )


def get_gradient_logvars(sess, xs, ys, samples=1000):
    all_qgrad = []
    all_pgrad = []
    for i in range(samples):
        if epoch > start:
            choose_samples = mis_cache(sess, xs, ys, update=False)
        else:
            choose_samples = mis_no_cache(sess, xs)
        flat_qgrad_temp, flat_pgrad_temp = sess.run(
            [flat_qgrad, flat_pgrad], {x: xs, ntimes: n, b_sample_in: choose_samples}
        )
...
train_data._labels = np.arange(
    0, len(train_data._labels)
)  # labels records the index of each training datapoint, for recording the cache samples
...
sess.run(init_op)
caches = np.zeros((len(train_data._labels), n_cv, n_class))
best_valid = 1e4
saver = tf.train.Saver()
start = 300  # epoch number to switch from JSA without cache to JSA with cache
...
for epoch in range(training_epochs):
    if log_variance and (epoch % 50 == 0 or epoch == training_epochs - 1):
        train_xs, train_ys = train_data.next_batch(batch_size)
        qlogv, plogv = get_gradient_logvars(sess, train_xs, train_ys)
    ...
    for i in range(total_batch):
        train_xs, train_ys = train_data.next_batch(batch_size)
        if epoch >= start:
            choose_samples = mis_cache(sess, train_xs, train_ys)
        else:
            choose_samples = mis_no_cache(sess, train_xs)
        _, cost = sess.run(
            [train_op, gen_loss], {x: train_xs, ntimes: n, b_sample_in: choose_samples}
        )
    ...
    if (epoch + 1) % 5 == 0:
        ...
        print(
            f"epoch:{epoch+1} time={(time.time()-begin)/60:.3f} train NLL={train_nll:.3f} valid NLL={valid_nll:.3f} test NLL={test_nll:.3f}"
        )
        begin = time.time()
